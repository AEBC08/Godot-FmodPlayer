...
namespace godot {
	...
	void FmodAudioBusLayout::_sync_bus_effects(Ref<FmodAudioBus> bus, int audio_server_bus_index) {
		...
		// int effect_count = audio_server->get_bus_effect_count(audio_server_bus_index);
	}
	...
	void FmodAudioBusLayout::sync_from_audio_server() {
		...
		UtilityFunctions::print("FMOD bus layout synchronized with AudioServer.");
	}
