#include "fmod_audio_effect_distortion.h"
...
void FmodAudioEffectDistortion::_bind_methods() {
	...
	ClassDB::add_property("FmodAudioEffectDistortion", PropertyInfo(Variant::INT, "mode", PROPERTY_HINT_ENUM, "Clip,Atan,LoFi,Overdrive,Waveshape,BitCrush"), "set_mode", "get_mode");
	...
	ClassDB::add_property("FmodAudioEffectDistortion", PropertyInfo(Variant::FLOAT, "pre_gain", PROPERTY_HINT_RANGE, "-60,24,0.1,suffix:dB"), "set_pre_gain", "get_pre_gain");
	...
	ClassDB::add_property("FmodAudioEffectDistortion", PropertyInfo(Variant::FLOAT, "drive", PROPERTY_HINT_RANGE, "0,1,0.01"), "set_drive", "get_drive");
	...
	ClassDB::add_property("FmodAudioEffectDistortion", PropertyInfo(Variant::FLOAT, "post_gain", PROPERTY_HINT_RANGE, "-60,24,0.1,suffix:dB"), "set_post_gain", "get_post_gain");
	...
	ClassDB::add_property("FmodAudioEffectDistortion", PropertyInfo(Variant::FLOAT, "keep_hf_hz", PROPERTY_HINT_RANGE, "1000,20000,1,suffix:Hz"), "set_keep_hf_hz", "get_keep_hf_hz");
	...
	ClassDB::add_property("FmodAudioEffectDistortion", PropertyInfo(Variant::INT, "oversample", PROPERTY_HINT_RANGE, "1,16,1"), "set_oversample", "get_oversample");
}
...
void FmodAudioEffectDistortion::remove_from_bus(Ref<FmodChannelGroup> p_bus) {
	if (p_bus.is_valid()) {
		for (int i = 0; i < dsp_chain.size(); i++) {
			if (dsp_chain[i].is_valid()) {
...
Dictionary FmodAudioEffectDistortion::get_parameters() const {
	Dictionary params;
	params["mode"] = (int)mode;
	params["pre_gain"] = pre_gain;
	params["drive"] = drive;
	params["post_gain"] = post_gain;
	params["keep_hf_hz"] = keep_hf_hz;
	params["oversample"] = oversample;
	return params;
}

void FmodAudioEffectDistortion::set_parameter(const String& name, float value) {
	if (name == "mode") {
		set_mode((Mode)(int)value);
	} else if (name == "pre_gain") {
		set_pre_gain(value);
	} else if (name == "drive") {
		set_drive(value);
	} else if (name == "post_gain") {
		set_post_gain(value);
	} else if (name == "keep_hf_hz") {
		set_keep_hf_hz(value);
	} else if (name == "oversample") {
		set_oversample((int)value);
	}
}
